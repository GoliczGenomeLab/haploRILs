...
```{r BUILD VARIABLE-SIZE WINDOWS WITH MINIMUM BP AND SNP}
...
```{r DETERMINE THE MINIMUM SNP NUMBER PER WINDOW TO REACH THE AIM RESOLUTION}
...
```{r ASSIGN SCORES TO FOUNDER SNPS BASED ON SIMILARITY WITH THE DESCENDANT}
...
```{r SUM SCORES BY WINDOWS AND DETECT HIGHEST-SCORING FOUNDERS}
...
```{r FILTER HBLOCKS USING THE K-CONTEXT METHOD AND EXTRACT A TABLE WITH THE MOST-LIKELY FOUNDER PER HBLOCK}
...
```{r inferMostLikelyPathAlternative6}
...
```{r IMPUTE REPETITIVE AND SOLATED DISSENTING BLOCKS}
...
```{r BUILD VARIABLE-SIZED SLIDING WINDOWS WITH MINIMUM BP AND SNP}
getBinCoords = function(map, minimum_snps, minimum_distance) {
  binStart_bp = c()
  binEnd_bp = c()
  binStart_snps = c()
...
```{r inferMostLikelyPathAlternative1}
...
```{r inferMostLikelyPathAlternative2}
...
```{r inferMostLikelyPathAlternative3}
...
```{r inferMostLikelyPathAlternative4}
...
```{r inferMostLikelyPathAlternative5}
...
```{r RECORD CROSSOVER COORDINATES}
...
```{r IMPUTE REPETITIVE HAPLOBLOCKS}
# ABABACCC --> AAAAACCC
