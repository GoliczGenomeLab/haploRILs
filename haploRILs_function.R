...
filterBlocks = function(blocks) {
  ...
  blocks = as.data.frame(blocks) %>% mutate(startIndex = as.numeric(blocks$startIndex), endIndex = as.numeric(blocks$endIndex), K = as.numeric(blocks$K), nSnp = as.numeric(blocks$nSnp))
  ...
  blocks = blocks %>% select(id, chr, nSnp, K, blocksFiltered, block, startIndex, endIndex, startBP, endBP, mostLikelyFoundersFiltered, mostLikelyFounders, mostLikelyFounders2, blockSumScore1, blockSumScore2, blockSumScoreRest, startInterval, endInterval, startIntervalLength, endIntervalLength)
  intervalLengthDF =
    blocks %>%
    #group_by(id, nSnp, K, blocksFiltered) %>%
    group_by(id, nSnp, K, blocksFiltered, .groups = "drop") %>%
    slice(1) %>%
  ...
  blocks = left_join(as.data.frame(blocks), as.data.frame(intervalLengthDF), by =c("id", "nSnp", "K", "blocksFiltered")) %>% select(-startIntervalLength, -endIntervalLength)
  return(blocks)
}
